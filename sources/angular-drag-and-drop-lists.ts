 ...
 * Copyright (c) 2016 Andrey Efimov anothar@ya.ru
 * https://github.com/anothar/angular-drag-and-drop-lists
 *
 * License: MIT
 */
    ...
    function ($parse, $timeout, dndDropEffectWorkaround, dndDragTypeWorkaround) {
      return function (scope, element, attr) {
        ...
        element.on('dragstart', function (event) {
          ...
          var data = angular.toJson(scope.$eval(attr.dndDraggable));
          event.dataTransfer.setData("Text", data);
          // @anothar we can't rely on setData - unfortunately it doesn't work in some chromium based browsers (Vivaldi)
          ...
          dndDropEffectWorkaround.transferredObject = data;
        ...
        element.on('dragend', function (event) {
          ...
          if (dndDropEffectWorkaround.dropEffect == 'none' && event.dataTransfer.dropEffect && event.dataTransfer.dropEffect != 'none' &&
            dndDropEffectWorkaround.listNode) {
            //fix for stupid browsers which don't invoke drop
            var dropEvent = angular.copy(event);
            dropEvent.type = 'drop';
            dndDropEffectWorkaround.listNode.triggerHandler('drop', dropEvent);
          }
  ...
  .directive('dndList', ['$parse', '$timeout', 'dndDropEffectWorkaround', 'dndDragTypeWorkaround',
    function ($parse, $timeout, dndDropEffectWorkaround, dndDragTypeWorkaround) {
      return function (scope, element, attr) {
        ...
        element.bind(getAllEvents(element[0]), function (e) {
          console.log(e.type);
        });
        /**
        ...
        element.on('dragover', function (event) {
          event = event.originalEvent || event;
          if (!isDropAllowed(event)) return true;
          dndDropEffectWorkaround.listNode = element;
        ...
        element.on('drop', function (event) {
          ...
          var transferredObject = dndDropEffectWorkaround.transferredObject;
         ...
         */
        function isMouseInFirstHalf(event, targetNode, relativeToParent = true) {
         ...
         */
        function invokeCallback(expression, event, index, item = null) {
